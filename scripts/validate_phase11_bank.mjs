...
const j1_2019_2 = eligible
  .filter((question) => question.year === 2019 && question.period === '2' && question.session === 'J1')
  .sort((a, b) => a.original_question_number - b.original_question_number);
const j1_2019_2_answerKey = [
...
if (j1_2019_2.length !== 80
  || j1_2019_2.filter((question) => question.section === 'CL').length !== 40
  || j1_2019_2.filter((question) => question.section === 'RL').length !== 40
  || j1_2019_2.some((question, index) => question.original_question_number !== index + 1)) {
  throw new Error('UDEA 2019-2 J1 must be a verified consecutive historical exam with 40 CL + 40 RL.');
}
if (j1_2019_2_answerKey.length !== 80
  || j1_2019_2.some((question, index) => question.correct_answer !== j1_2019_2_answerKey[index])) {
  throw new Error('UDEA 2019-2 J1 does not match the independently transcribed 80-answer key.');
}
for (const [number, expectedVisuals] of new Map([
  [43, 1], [47, 1], [60, 1], [61, 1], [62, 1], [63, 1], [64, 1],
  [65, 1], [66, 1], [67, 1], [69, 1], [73, 1], [74, 1], [78, 1], [79, 3],
])) {
  const question = j1_2019_2.find((item) => item.original_question_number === number);
  if ((question?.visual_resources?.length ?? 0) !== expectedVisuals) {
    throw new Error(`UDEA 2019-2 J1 question ${number} does not have its complete verified visual material.`);
  }
}
