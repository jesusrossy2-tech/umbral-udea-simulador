...
export async function GET(request: Request) {
  ...
  if (!validLearner(learnerId) || !['full','historical','practice'].includes(mode)) return Response.json({ error: 'Solicitud de examen inválida.' }, { status: 400 });
  ...
  if (mode === 'full') selected = buildFullExam(pool, seen);
    ...
    if (!selected.length) return Response.json({ error: 'No hay preguntas verificadas para esos filtros.' }, { status: 409 });
  } else {
    ...
    if (selected.length !== 80 || selected.filter((question) => question.section === 'CL').length !== 40) {
      return Response.json({ error: 'Este examen todavía no tiene 80 preguntas verificadas y no puede recrearse fielmente.' }, { status: 409 });
    }
  }
  const simulationId = crypto.randomUUID();
  await getD1().prepare(`INSERT INTO simulations (id, learner_id, mode, source_exam_id, question_ids_json, started_at, duration_seconds)
  ...
  return Response.json({ id: simulationId, mode, durationSeconds, sources: [...new Set(selected.map(examId))], questions: selected.map(publicQuestion) });
}
...
export async function POST(request: Request) {
  ...
  if (!body.simulationId || !body.learnerId || !validLearner(body.learnerId)) return Response.json({ error: 'Entrega inválida.' }, { status: 400 });
  const simulation = await getD1().prepare(`SELECT id, mode, source_exam_id, question_ids_json, duration_seconds, submitted_at FROM simulations
    WHERE id = ? AND learner_id = ?`).bind(body.simulationId, body.learnerId).first<{id:string;mode:ExamMode;source_exam_id:string|null;question_ids_json:string;duration_seconds:number;submitted_at:string|null}>();
  if (!simulation || simulation.submitted_at) return Response.json({ error: 'El simulacro no existe o ya fue entregado.' }, { status: 409 });
  ...
  const elapsedSeconds = Math.min(simulation.duration_seconds, Math.max(0, body.elapsedSeconds ?? 0));
  ...
  await db.batch([
    ...
    db.prepare(`INSERT INTO attempts
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(attemptId, body.learnerId, simulation.source_exam_id || simulation.id, savedAt, elapsedSeconds, body.timedOut ? 1 : 0,
  ...
  return Response.json({ status: body.timedOut ? 'timed_out' : 'submitted', mode: simulation.mode, elapsedSeconds, totals, sections, attemptId, savedAt, historySaved: true,
    standardizedScoring: { available: false, score: null, reason: 'No disponible con suficiente evidencia para reproducir fielmente el método oficial.' }, details });
}
